...
The lachesis protocol allows a group of participants to agree upon the occurrence and ordering of 
events over a network. The lachesis consensus protocol can be used to implement the common 
knowledge abstraction layer in the Fantom framework. The participants must know each other and agree upon the total number of participants in the network. Participants submit events to the network and the network comes 
...
The participants in the network are mutually agreed upon. Participants are identified by a unique public key. Each participant must store the public key, the IP address and the Creator's Id of every other participant in the network to disk. 

A participant may request another participant for a list of participants. The receiving participant must respond with a list of participants it knows.
...
A client sends a payload and a unique payload identifier to the Participant. The participant can create an event block using the payload. If the participant has accepted the payload, the participant will respond with message indicating the payload is pending. If the participant decides to reject the payload, the participant must respond with a rejected message indicating that the payload is 

### Client Authentication Process

Undefined in golang implementation

A client can updates on the status of their submitted payload by subscribing to messages from a participant. 
...
4. Creator's Public Key: the 256 bit public key associated with the participants private key
...
1. Participant's ID: the participants public key
...
A participant may send a synchronise events request to another participant. The requesting participant sends a list of events to the the participant it wishes to synchronise with. The responding participant responds with a 
...
A participant "gossips" by randomly selecting two other participants and sending them a sync request. The participants respond with a sync response.
...
An event that has ancestor events that have been created by 2/3 of the participants is a super-ancestored event.
...
Event that have occurred prior to the Atropos block are final. 
